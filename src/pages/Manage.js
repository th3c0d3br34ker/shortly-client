        ...
        {results?.length === 0 ? null : (
          <>
            <div className='w-full bg-white border flex flex-wrap items-center justify-between my-4 p-4 rounded text-sm'>
              ...
              <p className='md:w-auto mr-80 text-blue-400 flex flex-col md:flex-row md:items-center'>Short URL</p>
            </div>

            {results?.map((result, idx) => (
              <div className='w-full'>
                <UrlItem result={result} key={result.id + idx} refresh={refresh} />
              </div>
            ))}
          </>
        )}
