...
const FormInput = () => {
  ...
  const formHandler = async (evt) => {
    ...
    if (url === '') {
      ...
      const response = await createShortUrl({ original_url: url });
        ...
        <div className='container py-4 mx-auto max-w-screen-dt'>
          ...
          >
            <div className='w-full relative flex flex-col mb-8 lg:mb-0 '>
              ...
              />
              {error && <p className='absolute text-red-500 error-msg'>{error}</p>}
            </div>
            <button
          ...
          {result && (
            <div className='shorten__results'>
              <UrlItem result={result} key={result.id} refresh={() => setResult(null)} />
            </div>
          )}
        </div>
      </section>
